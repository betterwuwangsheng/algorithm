...
void copy() {
    ...
    for (i = 0; i < n; i++) {
        fscanf(sfp, "%20s%30s%10s%10d%5f\n", temp[i].no, temp[i].name,
               temp[i].sex, &temp[i].age, &temp[i].score);  //读入记录
        fprintf(tfp, "%-20s%-30s%-10s%-10d%5f", temp[i].no, temp[i].name,
                temp[i].sex, temp[i].age, temp[i].score);  //写入记录
        fprintf(tfp, "\r\n");                              //写入换行符
    }
    fclose(sfp);                       //关闭源文件
    fclose(tfp);                       //关闭目标文件
...
void aboutSystem() {
    ...
    printf(
        "|                                                                |\n");
    printf("|          ");
    printf("今天是   %4d年%2d月%2d日  星期%d  %2d时%2d分%2d秒", sys.wYear,
